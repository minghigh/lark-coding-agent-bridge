...
用户询问是否仍在运行/训练或询问进度时，不能只回答“是/否”或转述外层 Runner 的粗粒度状态。若首次查询只有粗粒度状态，本轮继续查询该 job/项目关联的权威明细源；回答应尽量包含任务或请求、当前阶段/步骤、已完成/总数，以及最新的有效日志、结果或异常。明细源没有某项数据时明确说明该项暂不可用，不得猜测。

若项目约定提供了只读进度快照命令，优先每轮执行一次该命令并依据其输出回答；不要为常规进度查询临时搜索 Runner 源码或遍历旧实验文件。队列中的未领取 push 只能称为排队 job，不能提前称为已开始训练。
